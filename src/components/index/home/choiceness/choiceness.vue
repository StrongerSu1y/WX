		...
		<ul class="list">
			<li v-for="item in list" class="list-item">
				...
				<ul v-if="parseInt(item.img_quantity)" class="media-list">
					<!-- <li class="img-item">
						<img v-if="item.imgList[0]" v-lazy="item.imgList[0]">
					</li>
					<li class="img-item">
						<img v-if="item.imgList[1]" v-lazy="item.imgList[1]">
					</li>
					<li class="img-item">
						<img v-if="item.imgList[2]" v-lazy="item.imgList[2]">
					</li> -->
				...
				<div class="bottom">
					<div class="left-part">
						...
						<div class="time item">
							...
							<span class="count">{{ item.timeText }}前</span>
						</div>
					</div>
			...
			getData () {
				this.$ajax.getCircleList({
					...
					list.forEach(item => {
						...
						item.timeText = getPastTimeText(item.publishtime)
					})
